...
func (e *ExasolDB) OpenDB(password string) (*sql.DB, error) {
	...
	log.WriteLog(1, e.Instance(), "Host: "+e.Host(), "Port: "+strconv.Itoa(e.Port()), "User: "+e.User(), "Schema: "+e.Schema(), "Table: "+tableFilter)
	db, err := sql.Open("exasol", exasol.NewConfig(e.User(), password).Port(e.Port()).Host(e.Host()).ValidateServerCertificate(false).String())
	if err != nil {
		log.WriteLog(1, e.Instance(), err.Error())
...
func (e *ExasolDB) QueryDB(db *sql.DB) error {
	...
	if logTableNamesFalse != constant.EMPTYSTRING {
		...
		log.WriteLog(1, e.Instance(), message)
	...
	for _, table := range tableNames {
		...
		rowSet, err = db.Query(sqlPreparedStmt, strings.ToUpper(e.Schema()), strings.ToUpper(table))
		if err != nil {
			log.WriteLog(1, e.ObjId(&table), err.Error())
			log.WriteLogBasic(constant.STDOUT, err.Error())
			return err
		}
		...
		log.WriteLog(2, e.ObjId(&table), "COLUMNS: "+logColumns, "DATATYPES: "+logColumnTypes)
		...
		log.WriteLog(2, e.ObjId(&table), "SQL: "+sqlQueryStmt)
		...
		log.WriteLog(1, e.ObjId(&table), result)
